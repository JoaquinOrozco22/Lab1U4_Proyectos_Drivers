 ...
 * 04/01/2023
 * Archivo: main.c
 * Programa de ejemplo usando lo drivers diseñados.
 * Es un sistema de control de temperatura, empleando un sensor LM235.
 * Cuando la temperatura sea mayor que el setpoint se activara una señal que enecndera 2 ventiladores para bajar
 * la temperatura. Esto se hace por medio de configuración y lectura de un unico canal.
 * Tambien se agregó la lectura de varios canales para el uso de las funciones de inicializado secuencial y
 * lectura secuencial.
 *
 *
 * Desarrollados por:
...
void app_main(void)
{
    ...
    while (true) {
		...
		temperatura=(((lectura_adc1_canal_6*VOLTAJE2)/RESOLUCION_12BITS_VAL_DECIMAL)*100)-339;
		...
		printf("\nSetpoint: %d\n", setpoint);
		printf("Temperatura Canal 6 ADC2: %.2f Grados\n",temperatura);
